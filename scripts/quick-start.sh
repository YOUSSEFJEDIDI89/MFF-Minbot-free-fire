...
# --------------------------------------------------------------------------- #
# Step 4: generate secrets (once, unless VORTEX_ADMIN_PASS was provided)
# --------------------------------------------------------------------------- #
step "4/7" "generating secrets (first run only)..."
if [ ! -f "$SECRET_FILE" ]; then
  # Use provided password if any, else generate a random one
  if [ -n "$CUSTOM_PASS" ]; then
...
elif [ -n "$CUSTOM_PASS" ]; then
  # File exists but user passed a new password — update it
  ADMIN_PASS="$CUSTOM_PASS"
  WEB_SECRET="$(grep '^VORTEX_WEB_SECRET=' "$SECRET_FILE" | cut -d= -f2-)"
  cat > "$SECRET_FILE" <<EOF
# Auto-generated by quick-start.sh. Keep this file private.
VORTEX_ADMIN_USER=admin
VORTEX_ADMIN_PASS=$ADMIN_PASS
...
EOF
  chmod 600 "$SECRET_FILE"
  ok "secrets file updated with your custom password"
else
  ok "secrets already exist"
fi
# shellcheck disable=SC1090
source "$SECRET_FILE"
# Export so bootstrap.py picks up the password
export VORTEX_ADMIN_USER VORTEX_ADMIN_PASS VORTEX_WEB_SECRET
